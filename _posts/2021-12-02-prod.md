...
To evaluate the effect of a given recommender algorithm on echo chambers and polarization using PROD, we have proposed the following evaluation methodology:
   ...
   6. Assess the average difference $∆m$ between the two cases. Since the initial measure $m(G)$ is the same with and without recommender, we obtain
   $$\Delta m = \frac{1}{K} \sum_{G ∈ Gμ,η} m(G^′_l)−m(G^′_0). $$
